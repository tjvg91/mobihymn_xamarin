...
namespace MobiHymn4.ViewModels
{
    public class BookmarksViewModel : BaseViewModel
    {
        ...
        public ICommand BookmarkGroupSelected
        {
            get
            {
                return bookmarkGroupSelected ?? (bookmarkGroupSelected = new Microsoft.Maui.Controls.Command<string>(async (key) =>
                {
                    await Shell.Current.GoToAsync(Routes.BOOKMARKS_LIST.Replace("{group}", Uri.EscapeDataString(key)));
                }));
            }
        }
